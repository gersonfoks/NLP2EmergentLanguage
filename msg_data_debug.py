...
for n in range(n_epochs):
    ...
    for msgs in train_dataloader:
        ...
        predictions, prediction_probabilities, hidden = rnn(msgs)

        prediction_probabilities = prediction_probabilities[:, :-1 , :].reshape(predictions.shape[0] * (predictions.shape[1] - 1), -1)
        predictions = predictions[:, :-1 , :].reshape(predictions.shape[0] * (predictions.shape[1] - 1), -1)
